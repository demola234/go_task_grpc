...
func runGRPCServer(configs utils.Config, store db.Store) {
	...
	pb.RegisterTodoGrpcServer(gRPCServer, server.UnimplementedTodoGrpcServer)
	// Register reflection service on gRPC server.
	reflection.Register(gRPCServer)b
...
func runGateWayServer(configs utils.Config, store db.Store) {
	...
	err = pb.RegisterTodoGrpcHandlerServer(ctx, gRPCMux, server.UnimplementedTodoGrpcServer)
	if err != nil {
	...
	if err != nil {
		fmt.Println("Cannot create stack system")
	}

	fs := http.FileServer(staticFx)
